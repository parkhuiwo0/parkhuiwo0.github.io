...
결국 Context Window 내에 들어오도록 설계할 때 성능 변화를 관찰하면 위 그래프에서 **성능이 거의 동일하게 나온다(곡선이 겹친다)**는 것을 확인할 수 있습니다.
(위 그래프에서 CPT-3.5-Trubo와 CPT-3.5-Trubo-16k 모델을 비교)

이는, 모델의 발전이 문맥 확장 기술(RoPE Scaling 등)이 입력 컨텍스트 내에서 정보 추출하는 근본적인 메커니즘을 개선하지 못했다는 점을 시사하기도 합니다.
