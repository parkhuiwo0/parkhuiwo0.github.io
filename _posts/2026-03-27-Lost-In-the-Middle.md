...
이 논문의 연구가 2023년도에 이루어져 빠르게 발전된 현재와는 안 맞을 수 있다고 생각할 수 있지만, 지속적으로 더 큰 Context Window를 가진 언어모델이 등장하더라도, 여전히 **트랜스포머(Transformer)기반의 아키텍처**가 다운스트림 태스크 수행 시 입력 컨텍스트를 어떻게 활용하는지에 대해서는 여전히 불문명하며 근본적인 한계가 존재한다. <br>
...
"언어 모델이 입력된 정보를 견고하게(Robustly) 활용한다면, 정보의 위치에 관계없이 성능이 일정하게 유지되어야 한다."
