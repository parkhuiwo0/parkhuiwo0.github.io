...
입력 프롬프트에서는 "제공된 검색 결과만을 사용하라"는 지시를 통해 사전 학습된 데이터에 의존하지 않고, 오직 주어진 컨텍스트 내에서만 추론하게 만듭니다. LLM 모델이 찾아야하는 정답은 **가운데인 문서[2]**에 있습니다. 나머지 문서들은 `Hard Negative`를 위한 유사하지만 정답이 없는 문서를 제공합니다.
