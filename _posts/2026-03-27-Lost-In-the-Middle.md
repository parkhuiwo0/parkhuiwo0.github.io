---
layout: post
title: "Lost In The Middle - How Language Models Use Long Contexts 논문 리뷰와 생각"
...
- [다중 문서 질의응답 실험 (Multi-Document Question Answering)](#다중-문서-질의응답-실험-multi-document-question-answering)
  - [실험 설정 (Experimental Setup)](#실험-설정-experimental-setup)
  - [실험 대상 모델](#실험-대상-모델)
  - [실험 요약 (통제변수 수정 예시)](#실험-요약-통제변수-수정-예시)
    - [Experimental 1. 정답 문서가 가운데에 있는 경우](#experimental-1-정답-문서가-가운데에-있는-경우)
    - [Experimental 2. 정답 문서가 첫번째에 있는 경우](#experimental-2-정답-문서가-첫번째에-있는-경우)
    - [Experimental 3. 입력 컨텍스트의 길이를 조정](#experimental-3-입력-컨텍스트의-길이를-조정)
  - [실험 결과](#실험-결과)

- [입력 컨텍스트로부터의 검색 능력 평가 실험 (How Well Can Language Models Retrieve From Input Contexts?)](#입력-컨텍스트로부터의-검색-능력-평가-실험-how-well-can-language-models-retrieve-from-input-contexts)
  - [실험 설정 (Experimental Setup)](#실험-설정-experimental-setup-1)
  - [Experimental](#experimental)
  - [실험 결과](#실험-결과-1)

- [왜 이런 현상이 발생할까요?](#왜-이런-현상이-발생할까요)
  - [모델 아키텍처의 영향 (Effect of Model Architecture)](#모델-아키텍처의-영향-effect-of-model-architecture)
  - [쿼리 인식 문맥화(Query-Aware Contextualization)의 영향](#쿼리-인식-문맥화query-aware-contextualization의-영향)
  - [지시어 미세 조정(Instruction Fine-Tuning)의 영향](#지시어-미세-조정instruction-fine-tuning의-영향)

- [컨텍스트가 길수록(많을수록) 항상 좋을까요? (Is More Context Is Always Better?)](#컨텍스트가-길수록많을수록-항상-좋을까요-is-more-context-is-always-better)

- [Related Work](#related-work)
  - [Long-Context Language Models](#long-context-language-models)
  - [언어 모델은 Context를 어떻게 활용하는가? (How Do Language Models Use Context?)](#언어-모델은-context를-어떻게-활용하는가-how-do-language-models-use-context)
  - [서열 위치 효과 (The Serial-Position Effect)](#서열-위치-효과-the-serial-position-effect)
...
이 실험의 재미있는 포인트는, 단순하게 긴 텍스트(컨텍스트)를 입력하는 것이 아닌, **RAG 시스템에서 흔히 발생하는 '검색된 문서 나열' 상황을 재현**하여 실험했다는 점이에요. 또한, 모델이 출력한 답변에 실제 정답이 포함되어 있는지를 측정하는 정확도(Accuracy)를 주요 지표로 삼았습니다. <br>
...
위와 같이 어떻게 보면 단순해 보일 수 있지만, [NIAH(Needle in a Haystack)](https://arxiv.org/abs/2406.07230) 연구와 비슷하게 위에서 언급한 두 가지 변수를 Finer-grained 조정을 통해 실험하며 굉장히 디테일한 패턴을 분석하였습니다.
...
(위 그래프에서 CPT-3.5-Turbo CPT-3.5-Turbo-16k 모델을 비교)
...
그렇다면, 입력 문맥에서 단순히 정보를 찾아내는 것(Retrieve)은 어느 정도의 성능을 보여주는지에 대해 검증하기 위해 synthetic key-value를 설계하여 실험을 진행했다고 합니다.
...
자연어는 모델이 사전 학습된 데이터에 의존할 수 있기 때문에, Zero-Semantics 데이터에서 오직 토큰 매칭 능력만으로 답을 찾아내야 하기 때문에 훨씬 더 엄격하게 테스트할 수 있습니다. (즉, 의미 이해가 아니라 순수하게 위치 기반 검색 능력만을 평가하는 설정입니다.)
...
한 가지 흥미로운 사실은, LongChat 모델은 값을 말하지 않고 코드를 생성했다는 점인데요, 이는 모델이 **긴 데이터를 처리하는 상황을 마주했을 때 학습 데이터 속의 특정 패턴(데이터를 처리할 때 코드를 작성하는 예시)에 편향되어 반응**했을 가능성도 존재합니다.
...
위의 실험들에서 입력창을 예시로 보여드린 것을 다시 한번 보고 오시는 것을 추천드립니다. 모두 사용자의 질문을 문서/정보의 뒤쪽에 배치했습니다.
그 결과, GPT와 같은 Decoder-Only 모델은 문서를 읽는 동안 질문이 무엇인지 알 수 없었습니다. 디코더 모델은 앞서 설명드린 것과 같이 `Causal Attention`으로 다음에 나올 내용에 대해서 알지 못합니다.
...
### 실험 결과
단순 키-값 검색(입력 컨텍스트로부터의 검색 능력 평가 실험 (How Well Can Language Models Retrieve From Input Contexts?))에 대해서는 엄청난 효과가 발생했다고 합니다. 모든 모델이 거의 완벽에 가까운 성능을 보였고, GPT-3.5-Turbo(16K)는 300개의 데이터 사이에서도 **100% 정답**을 기록했다고 합니다. (기존 최악의 성능은 45.6%)
...
이를 실험해 보기 위해 Fine-Tuning 전의 MPT-30B(Base) 모델과 조정 후의 MPT-30B-Instruct를 비교해 봤다고 해요.
...
즉, 학습 방식([RLHF](https://en.wikipedia.org/wiki/Reinforcement_learning_from_human_feedback) 등)의 문제라기보다, 모델이 거대한 인터넷 데이터를 학습하며 스스로 습득한 **'구조적 특징'**에 가까울 수 있을 것 같아요.
...
위의 모든 실험 과정에서 살펴볼 수 있듯이, 긴 컨텍스트를 사용하는 것은 트레이드오프(Trade-off) 관계임을 보여줍니다.
...
하지만, 긴 컨텍스트를 처리하는 능력과 **긴 컨텍스트에서 정확한 정보를 찾아 접근하여 결과를 도출하는 능력은 또 다른 연구 주제**임을 이 논문을 통해서 증명했습니다.
...
Transformer 모델의 시대에서도 상황은 비슷합니다. 모델이 컨텍스트 전체를 정교하게 읽는다기보다는, 특정 패턴에 의존하고 있을 수 있다고 합니다.
최근의 효율적인 트랜스포머 기반의 모델들 역시 비슷한 현상이 관찰되고 있기도 합니다.

### 서열 위치 효과 (The Serial-Position Effect)
심리학에는 [서열 위치 효과(Serial-Position Effect)](https://en.wikipedia.org/wiki/Serial-position_effect)라는 연구 결과가 있습니다.
...
현대 [트랜스포머](https://namu.wiki/w/%ED%8A%B8%EB%9E%9C%EC%8A%A4%ED%8F%AC%EB%A8%B8(%EC%9D%B8%EA%B3%B5%EC%8B%A0%EA%B2%BD%EB%A7%9D)) 기반의 LLM 모델들은 위 논문과 같이 입력 컨텍스트의 길이와 정보의 위치에 따라 처리 성능이 달라지는 현상이 있습니다.
...
이 논문의 가장 큰 공헌은 컨텍스트 처리 크기와 처리 능력을 구분하여 **컨텍스트의 처리 능력이 더욱더 발전해야 하는 과제로 남아있다**는 것을 증명했다는 점이라고 생각합니다.
...
# 참고 자료
- [Lost in the Middle: How Language Models Use Long Contexts, 2023 Nelson F. Liu, Kevin Lin, John Hewitt, Ashwin Paranjape, Michele Bevilacqua, Fabio Petroni, Percy Liang](https://arxiv.org/abs/2307.03172)
